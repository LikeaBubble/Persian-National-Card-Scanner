...
class inspection:
    ...
    def validate(self,input):
        out = input
        if not self.is_only_numbers(input['id']):
            out['national_code'] = 'NotRecognized'
        if not self.is_date_format(input['expire_date']):
            out['expire_date'] = 'NotRecognized'
        if not self.is_date_format(input['birth_date']):
            out['birth_date'] = 'NotRecognized'
        if not self.is_only_letters(input['first_name']):
            out['first_name'] = 'NotRecognized'
        if not self.is_only_letters(input['last_name']):
            out['last_name'] = 'NotRecognized'
        if not self.is_only_letters(input['father_name']):
            out['father_name'] = 'NotRecognized'
        return out
    
    def is_date_format(self,text):
        if text=='NotRecognized' or text=='NotDetected':
            return False
